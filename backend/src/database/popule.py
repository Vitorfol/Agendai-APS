...
def popular_banco():
    ...
    try:
        ...
        session.flush()  # garante ids para universidades

        # --- Usuarios ---
        user_prof = Usuario(
            nome="Dr. Roberto Silva",
            email="roberto@uft.edu.br",
            cpf="11111111111",
            senha=pegar_senha_hash("senha123")
        )
        user_aluno1 = Usuario(
            nome="Ana Souza",
            email="ana.souza@aluno.uft.edu.br",
            cpf="22222222222",
            senha=pegar_senha_hash("senha123")
        )
        user_aluno2 = Usuario(
            nome="Carlos Lima",
            email="carlos.lima@aluno.uft.edu.br",
            cpf="33333333333",
            senha=pegar_senha_hash("senha123")
        )
        user_convidado = Usuario(
            nome="Palestrante Externo",
            email="palestrante@externo.com",
            cpf="44444444444",
            senha=pegar_senha_hash("senha123")
        )
        session.add_all([user_prof, user_aluno1, user_aluno2, user_convidado])
        session.flush()  # garante ids para usuarios

        # --- Professor (PK = id_usuario) ---
        prof = Professor(
            id_usuario=user_prof.id,        # chave primária vem do usuario.id
        ...
        session.flush()  # garante ids para cursos

        # --- Alunos (PK = id_usuario) ---
        aluno1 = Aluno(
            id_usuario=user_aluno1.id,
            id_curso=curso_bcc.id,
            matricula="2023001"
        )
        aluno2 = Aluno(
            id_usuario=user_aluno2.id,
            id_curso=curso_bcc.id,
            matricula="2023002"
        )
        ...
        # --- Eventos (note: email_proprietario referencia usuario.email) ---
        evento_palestra = Evento(
            nome="Palestra sobre Inovações Tecnológicas",
            descricao="Uma palestra sobre inovações tecnológicas.",
            id_universidade=u1.id,
            data_inicio=datetime.now() + timedelta(days=1),
            data_termino=datetime.now() + timedelta(days=1, hours=2),
            recorrencia="Não",
            categoria="Palestra",
            email_proprietario=user_prof.email  # FK -> usuario.email
        )
        evento_aula = Evento(
        ...
        session.flush()  # gera ids para eventos

        # --- Disciplina (PK = id_evento) ---
        # Obs: professor.id_usuario já contém user_prof.id
        disciplina = Disciplina(
            id_evento=evento_aula.id,          # PK refere ao evento
            id_professor=prof.id_usuario,      # FK -> professor.id_usuario
            horario="19:00",
        ...
        curso_disc = CursoDisciplina(
            id_curso=curso_bcc.id,
            id_disciplina=disciplina.id_evento,
            creditos=4
        )
        ...
        notificacao = Notificacao(
            id_usuario=user_aluno1.id,     # FK -> usuario.id
            data=datetime.now(),
            mensagem="Aula de BD I vai começar",
            evento=str(evento_aula.id)    # mantém campo evento como string (id ou descrição)
        )
        session.add(notificacao)

        # --- Convidado ---
        convidado = Convidado(
            id_evento=evento_palestra.id,
            id_usuario=user_convidado.id
        )
        session.add(convidado)

        # --- Presencas (id_aluno refere-se a aluno.id_usuario) ---
        presenca1 = Presenca(
            id_ocorrencia_evento=ocorrencia.id,
            id_aluno=aluno1.id_usuario,
            presente=True
        )
        presenca2 = Presenca(
            id_ocorrencia_evento=ocorrencia.id,
        ...
        print(f"Usuarios: prof={user_prof.id}, aluno1={user_aluno1.id}, aluno2={user_aluno2.id}, convidado={user_convidado.id}")
        print(f"Eventos: aula={evento_aula.id}, palestra={evento_palestra.id}")
        print(f"Disciplina (id_evento): {disciplina.id_evento}")
